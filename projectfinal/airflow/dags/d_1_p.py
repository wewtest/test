...
from modules.connection import Connections


def fun_get_data_from_api(**kwargs):
    # get data
    #scraper = Scrapers(Variable.get('url_covid_tracker'))
    #data = scraper.get_data()
    #print(data.info())
    url='http://103.150.197.96:5001/api/v1/rekapitulasi_v2/jabar/harian?level=kab'
    tmpresponse = requests.get(url)
    tmpresult = tmpresponse.json()['data']['content']
    data = pd.json_normalize(tmpresult)
